...
* **Large Language Models (LLMs):** Central to Actor cognition and hierarchical control. The existing `llm_provider.py` (supporting OpenRouter, LM Studio) will be retained and integrated.
...
## 3. * **Project Glossary:** A document will be maintained to define these and other key terms as they are solidified, ensuring consistent usage across the project.
...
* **LLM Services (`/engine/llm_services/`)**
    * Integration of the existing `llm_provider.py`.
...
3.  **LLM Service Integration:** Integrate `llm_provider.py`.
4.  **Basic `Actor` Class:**
